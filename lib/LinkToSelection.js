...
(function(){
  ...
  function onSelectionChange(e) {
    ...
    if (oldOnSelectionChange) oldOnSelectionChange(e);
  }
  ...
  if (!String.prototype.startsWith) { // assuming it's IE
  ...
  if (window.getSelection) {
    ...
    // Firefox will support document.onselectionchange since version 45;
    // until then, use polling
    var m;
    if (m = navigator.userAgent.match(/Firefox\/(\d+)/)) {
      if (m[1] < 45) usePolling = true;
    }

    if (usePolling) setInterval(calculateSelection, SELECTION_TIMEOUT);

    if (typeof(jQuery) == 'undefined') {
      window.onload = updateFromHash;
    } else {
      $(document).ready(updateFromHash);
    }
  } else {
